(function(jQuery) {
	...
	jQuery.fn.stickyNotes.editNote = function(paragraph) {
		...
		jQuery("#note-" + note_id).find("textarea").focus();
		if( jQuery.fn.stickyNotes.options.maxLength )
		{
			jQuery("#note-" + note_id).find("textarea").keypress(function() {
				return jQuery(this).val().length > jQuery.fn.stickyNotes.options.maxLength ? false: true;
			});
		}
		jQuery.fn.stickyNotes.setCurrentlyEditedNote(note_id);
	}
